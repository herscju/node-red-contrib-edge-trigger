        ...
        this.on('input', function (msg) {
            if (msg.hasOwnProperty('payload')) {
                ...
                if (!isNaN(this_value)) {
                    ...
                    if (!isNaN(last_value)) {
                        if (this_value > last_value && this_value > config.rising_threshold && last_value <= config.rising_threshold && last_edge != 'rising') {
                            status(msg, true);
                        }

                        if (this_value < last_value && this_value < config.falling_threshold && last_value >= config.falling_threshold && last_edge != 'falling') {
                            status(msg, false);
                        }
                    } else { // first value
